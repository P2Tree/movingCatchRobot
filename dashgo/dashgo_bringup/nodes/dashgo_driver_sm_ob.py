...
class BaseController:
    def __init__(self, arduino, base_frame):
        ...
        # ping
        self.front_left_ranger=500
        self.front_ranger=500
        self.front_right_ranger=500
    ...
    def poll(self):
        ...
        if now > self.t_next:
            try:
                ...
                self.front_left_ranger=r1
                self.front_ranger=r2
                self.front_right_ranger=r3
                self.back_ranger=r4
                #  rospy.loginfo("ultrasonic sensor ranger: " + str(r1)+","+str(r2)+","+str(r3)+","+str(r4))
            except:
    ...
    def cmdVelCallback(self, req):
        ...
        if(self.front_ranger<=self.safe_ranger_0 or 
                self.front_left_ranger<=self.safe_ranger_0 or
                self.front_right_ranger<=self.safe_ranger_0) and (x>0):
            x=0
        elif(self.front_ranger<=self.safe_ranger_1 or
                self.front_left_ranger<=self.safe_ranger_1 or
                self.front_right_ranger<=self.safe_ranger_1) and (x>0.05):
            x=0.05
        elif(self.front_ranger<=self.safe_ranger_2 or
                self.front_left_ranger<=self.safe_ranger_2 or
                self.front_right_ranger<=self.safe_ranger_2) and (x>0.15):
            x=0.15
